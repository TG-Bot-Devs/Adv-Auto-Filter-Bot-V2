...
async def auto_filter(bot, update):
    ...
    if filters:
        for filter in filters: # iterating through each files
            ...
            if pm_file_chat: 
                ...
                file_link = f"https://t.me/{bot_.username}?ustart={unique_id}"
    ...
    if len(results) == 0: # double check
        ...
        # Just A Decaration
        result[0].append([
            InlineKeyboardButton(f"🔘 Nᴇxᴛ Pᴀɢᴇ /{len_result if len_result < max_pages else max_pages} 🔘", callback_data="ignore")
        ])
        ...
        try:
            await bot.send_message(
                chat_id = update.chat.id,
                text=f"<b>Found {(len_results)} Results. ʜᴇʀᴇ ɪꜱ ᴛʜᴇ ʀᴇꜱᴜʟᴛꜱ ꜰᴏʀ yᴏᴜʀ qᴜᴇʀy</b>: <code>{query}</code>",
